...
/* ── Streaming ASR (delegates to active backend) ─────────── */
...
voice_asr_stream_t* voice_asr_stream_open(void)
{
    ...
    if (strcmp(s_active->name, "dashscope") == 0) {
        ds_asr_stream_t* inner = dashscope_asr_stream_open();
        if (!inner) {
            return NULL;
        }
        voice_asr_stream_t* s = calloc(1, sizeof(*s));
        if (!s) {
            dashscope_asr_stream_abort(inner);
            return NULL;
        }
        s->inner = inner;
        s->is_dashscope = 1;
    ...
    const unsigned char* pcm, size_t len)
{
    ...
    if (s->is_dashscope) {
        return dashscope_asr_stream_send(s->inner, pcm, len);
    }
    ...
    char* text_out, size_t text_cap)
{
    ...
    int ret;
    if (s->is_dashscope) {
        ret = dashscope_asr_stream_finish(s->inner, text_out, text_cap);
    } else {
        ret = volc_asr_stream_finish(s->inner, text_out, text_cap);
    }
...
void voice_asr_stream_abort(voice_asr_stream_t* s)
{
    ...
    if (s->is_dashscope) {
        dashscope_asr_stream_abort(s->inner);
    } else {
...
int voice_asr_stream_vad_done(voice_asr_stream_t* s)
{
    if (!s || !s->inner)
        return 0;

    if (s->is_dashscope) {
        return dashscope_asr_stream_vad_done(s->inner);
    }
