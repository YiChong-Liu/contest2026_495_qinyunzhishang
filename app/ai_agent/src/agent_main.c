...
#ifdef CONFIG_NETUTILS_NTPCLIENT
/* Ensure NTP client is running and wait for actual time sync.
 *
 * The key insight: checking year >= 2025 is NOT sufficient to prove
 * NTP sync occurred — the RTC may already show 2025+ but be off by
 * tens of seconds due to crystal drift.
 *
 * Instead, we detect actual clock adjustment by comparing
 * CLOCK_REALTIME (adjustable by NTP via clock_settime) against
 * CLOCK_MONOTONIC (never adjusted).  Before NTP sync, both advance
 * at the same rate.  When NTP calls clock_settime(), the delta
 * between them changes, proving a sync actually happened.
 *
 * ntpc_start_with_list() is idempotent: if lvgldemo already started
 * the daemon, this call returns immediately without starting a
 * second instance. */
static void ntp_sync_wait(void)
{
    /* Record the initial relationship between REALTIME and MONOTONIC.
     * If NTP later adjusts CLOCK_REALTIME, the delta will shift. */
    struct timespec rt_start, mono_start;
    clock_gettime(CLOCK_REALTIME, &rt_start);
    clock_gettime(CLOCK_MONOTONIC, &mono_start);

    struct tm tm = agent_localtime();
    syslog(LOG_INFO, "[%s] Pre-NTP time: %04d-%02d-%02d %02d:%02d:%02d %s\n",
           TAG, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
           tm.tm_hour, tm.tm_min, tm.tm_sec, agent_get_timezone());

    /* Ensure NTP daemon is running (idempotent — safe even if
     * lvgldemo already started it). */
    int ret = ntpc_start_with_list(CONFIG_NETUTILS_NTPCLIENT_SERVER);
    if (ret < 0) {
        syslog(LOG_WARNING, "[%s] ntpc_start_with_list failed: %d\n", TAG, ret);
        return;
    }

    /* Wait up to 30 seconds for NTP to adjust the clock.
     * Detect sync by comparing elapsed REALTIME vs elapsed MONOTONIC.
     * If they diverge by >= 1 second, clock_settime was called. */
    for (int i = 0; i < 30; i++) {
        sleep(1);
        ...
        int64_t rt_elapsed = (int64_t)(rt_now.tv_sec - rt_start.tv_sec);
        int64_t mono_elapsed = (int64_t)(mono_now.tv_sec - mono_start.tv_sec);
        int64_t drift = rt_elapsed - mono_elapsed;
        if (drift < 0) drift = -drift;

        if (drift >= 1) {
            /* CLOCK_REALTIME was adjusted — NTP sync confirmed */
            tm = agent_localtime();
            syslog(LOG_INFO, "[%s] NTP sync OK (clock adjusted %llds): %04d-%02d-%02d %02d:%02d:%02d %s\n",
                   TAG, (long long)drift, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
                   tm.tm_hour, tm.tm_min, tm.tm_sec, agent_get_timezone());
            return;
        }
    }
    ...
    tm = agent_localtime();
    syslog(LOG_WARNING, "[%s] NTP sync timeout, current time: %04d-%02d-%02d %02d:%02d:%02d %s\n",
           TAG, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
           tm.tm_hour, tm.tm_min, tm.tm_sec, agent_get_timezone());
}
#endif /* CONFIG_NETUTILS_NTPCLIENT */
 ...
 *
 * If network is not available at startup, keeps retrying every 5s
 * until the link comes up (e.g. user configures WiFi later).
 */
static void* network_watch_task(void* arg)
{
    ...
    if (network_wait_connected(30000) != OK) {
        syslog(LOG_WARNING, "[%s] Network timeout — retrying every 5s.\n", TAG);
        syslog(LOG_WARNING, "[%s] Use 'config_show' / 'set_*' CLI commands to configure.\n", TAG);
        while (!g_shutdown_requested) {
            sleep(5);
            network_wifi_reconnect();
            if (network_wait_connected(0) == OK)
                break;
        }
    }
    ...
    /* Print current local time after NTP sync */
    {
        struct tm tm = agent_localtime();
        syslog(LOG_INFO, "[%s] *** Local time: %04d-%02d-%02d %02d:%02d:%02d %s ***\n",
               TAG, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
               tm.tm_hour, tm.tm_min, tm.tm_sec, agent_get_timezone());
    }
...
static void* outbound_dispatch_task(void* arg)
{
    ...
    while (!g_shutdown_requested) {
        ...
        if (message_bus_pop_outbound(&msg, 1000) != OK) {
            /* Log alive every 30 seconds to help diagnose stuck threads */
            if (++idle_secs % 30 == 0) {
                syslog(LOG_INFO,
                    "[%s] Outbound dispatch alive (idle %d s)\n",
                    TAG, idle_secs);
            }
            continue;
        }
        ...
        if (strcmp(msg.channel, AGENT_CHAN_FEISHU) == 0) {
            ...
            if (s_voice_cooldown && time(NULL) < s_voice_cooldown_until) {
                ...
                /* Broadcast to WS clients for display on external UI
                 * (e.g. lvgldemo) BEFORE TTS so the screen updates
                 * while the voice is still speaking.
                 *
                 * Use type "response_silent" so the lvgldemo ai_page
                 * displays the text but does NOT trigger its own TTS —
                 * voice_channel_speak() below is the sole TTS path for
                 * voice-channel replies.  This prevents dual TTS
                 * (ai_play_tts + voice_channel_speak) that caused
                 * overlapping audio and duplicate playback. */
                ws_server_broadcast_typed("response_silent", msg.content,
                                          AGENT_CHAN_VOICE);
...
int ai_agent_main(int argc, char* argv[])
{
    ...
    /* ── Phase 1: Core infrastructure ──────────────────────── */
    {
        ...
        if (AGENT_SECRET_API_KEY[0]) {
            char tmp[128] = { 0 };
            /* 文件优先级最高：/emmc/wifi/agent_app_id 存在时无条件覆盖 config.json，
             * 确保烧录不同文件即可切换 key，不受 config.json 中旧值影响。 */
            char file_key[128] = { 0 };
            if (claw_load_file_string("/emmc/wifi/agent_app_id",
                                      file_key, sizeof(file_key)) == OK) {
                claw_config_set(AGENT_CFG_KEY_API_KEY, file_key);
                syslog(LOG_INFO, "[%s] api_key synced from /emmc/wifi/agent_app_id\n", TAG);
            } else if (claw_config_get(AGENT_CFG_KEY_API_KEY, tmp, sizeof(tmp)) != OK
                       || tmp[0] == '\0') {
                /* 文件不存在且 config.json 无值时，用宏兜底 */
                claw_config_set(AGENT_CFG_KEY_API_KEY, AGENT_SECRET_API_KEY);
                syslog(LOG_INFO, "[%s] Bootstrapped api_key from AGENT_SECRET_API_KEY\n", TAG);
            }
            if (claw_config_get(AGENT_CFG_KEY_LLM_HOST, tmp, sizeof(tmp)) != OK
                || tmp[0] == '\0') {
                claw_config_set(AGENT_CFG_KEY_LLM_HOST, AGENT_LLM_QWEN_HOST);
                claw_config_set(AGENT_CFG_KEY_LLM_PATH, AGENT_LLM_QWEN_PATH);
                claw_config_set(AGENT_CFG_KEY_MODEL, AGENT_LLM_QWEN_MODEL);
                syslog(LOG_INFO, "[%s] Bootstrapped LLM config for DashScope\n", TAG);
            }
        }
    ...
    /* ── Phase 3: Application services ─────────────────────── */
    {
        ...
        {
            char api_tmp[128] = { 0 };
            if (claw_config_get(AGENT_CFG_KEY_API_KEY, api_tmp, sizeof(api_tmp)) == OK
                && (strncmp(api_tmp, "sk-", 3) == 0 || strncmp(api_tmp, "sk_", 3) == 0)) {
                voice_asr_set_backend("dashscope");
                voice_tts_set_backend("dashscope");
                syslog(LOG_INFO, "[%s] DashScope key detected, switched ASR/TTS to dashscope\n", TAG);
            }
        }
