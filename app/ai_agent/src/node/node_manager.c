...
static int send_event(int fd, const char* event, cJSON* payload)
{
    ...
    int ret = -1;
    if (json) {
        ret = ws_send_text_to_fd(fd, json, strlen(json));
        free(json);
    } else {
        syslog(LOG_WARNING, "[%s] send_event: cJSON_PrintUnformatted returned NULL\n", TAG);
    }
    cJSON_Delete(frame);
    return ret;
}
    ...
    cJSON* error)
{
    ...
    if (json) {
        ...
        syslog(LOG_WARNING, "[%s] send_response: cJSON_PrintUnformatted returned NULL\n", TAG);
    }
    cJSON_Delete(frame);
    return ret;
}
