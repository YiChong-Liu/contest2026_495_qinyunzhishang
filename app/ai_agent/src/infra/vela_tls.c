    ...
    size_t* out_body_len, bool* out_keep_alive)
{
    ...
    while (!eof && raw_len < TLS_RAW_BUF_SIZE - 1) {
        ...
        if (ret > 0) {
            raw_len += (size_t)ret;
            want_read_retries = 0;  /* got data, reset counter */
            ...
            /* WANT_READ: no data within SO_RCVTIMEO.  Allow 2 retries
             * (60s total with 30s timeout) before giving up. */
            if (++want_read_retries > 2) {
                syslog(LOG_ERR, "[%s] ssl_read (header) timeout after %d WANT_READ\n",
                       TAG, want_read_retries);
                tls_raw_release(raw);
                return VELA_TLS_ERR_READ;
            }
        }
    ...
    syslog(LOG_INFO, "[%s] http resp: chunked=%d content_length=%ld initial_body=%zu\n",
           TAG, chunked, content_length,
           (size_t)(raw + raw_len - body_start));
    ...
    /* Keep reading body */
    /* Early exit checks to avoid WANT_READ timeout:
     * 1. Content-Length known and initial read got the full body
     * 2. Chunked and initial read already contains the terminating chunk
     * Both are common for small API responses like Feishu. */
    int skip_body = 0;
    if (eof) {
        skip_body = 1;
    } else if (!chunked && content_length >= 0 && (long)resp_pos >= content_length) {
        skip_body = 1;
    } else if (chunked && resp_pos >= 5) {
        /* Check for last chunk: "0\r\n\r\n" at end of buffer */
        if (memcmp(resp_buf + resp_pos - 5, "0\r\n\r\n", 5) == 0)
            skip_body = 1;
    }
    if (!skip_body) {
        int body_retries = 0;
        while (resp_pos < resp_cap - 1) {
            ...
            if (ret > 0) {
                resp_pos += (size_t)ret;
                body_retries = 0;  /* got data, reset counter */
                /* Early exit: chunked body complete */
                ...
                if (++body_retries > 2) {
                    syslog(LOG_ERR, "[%s] ssl_read (body) timeout after %d WANT_READ\n",
                           TAG, body_retries);
                    break;
                }
            }
    ...
    size_t* out_body_len)
{
    ...
    if (slot && slot->valid) {
        ...
        if (slot->ctx.net.fd >= 0) {
            ...
            /* If peer closed the connection or drain timed out
             * (WANT_READ), reconnect.  A WANT_READ after a 10ms
             * timeout means no pending data — but the server may
             * have already sent a FIN that mbedTLS hasn't processed
             * yet.  Reconnecting is safer than risking a 120s
             * hang on a half-closed connection. */
            if (dr == 0 || dr == MBEDTLS_ERR_SSL_PEER_CLOSE_NOTIFY ||
                dr == MBEDTLS_ERR_SSL_WANT_READ) {
                goto pool_reconnect;
            }
        } else {
            goto pool_reconnect;
