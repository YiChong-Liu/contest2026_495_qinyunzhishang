...
/* TLS 连接池的串行化已由 vela_tls.c 的 s_pool_lock 保护，
 * 此处不再持有全局锁，避免在 getaddrinfo(DNS/UDP)/TCP/TLS 握手等
 * 阻塞 IO 期间长持锁导致与 LVGL epoll 的 fd 竞争窗口扩大。
 * token 共享变量由 feishu_bot.c 的 s_token_lock 保护。 */
