...
/* 全局网络锁：mbedtls 不是线程安全的，飞书 poll 线程(WS) 和发送线程(HTTPS)
 * 可能同时操作 mbedtls/TLS，必须串行化所有网络 I/O。 */
extern pthread_mutex_t s_feishu_net_lock;

/* 保护 s_access_token / s_token_expire_ts / s_bot_open_id 的读写（短持锁，不跨网络 IO） */
extern pthread_mutex_t s_token_lock;
...
/* 文档创建成功后通知上层回调（内部使用，由tool_feishu_doc_create调用） */
