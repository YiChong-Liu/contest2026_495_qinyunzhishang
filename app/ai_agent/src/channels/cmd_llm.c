...
static const router_preset_t g_router_presets[] = {
    ...
    /* Use workspace-dedicated domain for lower TLS handshake latency.
     * See agent_config.h AGENT_LLM_QWEN_HOST for rationale. */
    { "qwen", "dashscope.aliyuncs.com",
        "/compatible-mode/v1/chat/completions", "qwen3.5-flash", 1 },
...
{
    if (argc < 2) {
        printf("Usage: set_llm <preset> [model] [api_key]\n"
               "       set_llm <url> <model> [api_key]\n"
               "       set_llm <host> <model> [api_key]\n"
               "\n"
               "Presets:\n"
               "  kimi     - api.moonshot.cn  (kimi-k2.5)\n"
               "  qwen     - dashscope.aliyuncs.com  (qwen3.5-flash)\n"
               ...
               "\n"
               "Examples:\n"
               "  set_llm qwen qwen3.6-plus sk-xxx\n"
    ...
    /* Check presets — reuse g_router_presets table */
        ...
        if (preset) {
            ...
            port = "443";
            if (argc >= 3) {
                if (strncmp(argv[2], "sk-", 3) == 0 || strncmp(argv[2], "sk_", 3) == 0) {
                    api_key = argv[2];
                } else {
                    model = argv[2];
                }
