...
static int do_ws_handshake(int fd, char* chat_id_out, size_t chat_id_size)
{
    ...
    while (total < (int)sizeof(buf) - 1) {
        ...
        if (n < 0 && (errno == EAGAIN || errno == EWOULDBLOCK))
            continue;
        if (n <= 0) {
            syslog(LOG_WARNING, "[%s] Handshake recv failed fd=%d n=%d errno=%d\n",
                TAG, fd, n, errno);
            return -1;
        }
        total += n;
    ...
    syslog(LOG_INFO, "[%s] Handshake request fd=%d (%d bytes)\n", TAG, fd, total);
    ...
    if (send(fd, resp, rlen, 0) != rlen) {
        syslog(LOG_WARNING, "[%s] Handshake send 101 failed fd=%d\n", TAG, fd);
        return -1;
    }

    syslog(LOG_INFO, "[%s] Handshake 101 sent fd=%d (%d bytes)\n", TAG, fd, rlen);
...
/**
 * Read exactly `count` bytes from fd using poll+recv (avoids MSG_WAITALL
 * which may not work reliably on all NuttX socket backends).
 * Returns 0 on success, -1 on error/timeout.
 */
static int recv_exact(int fd, void* buf, size_t count)
{
    size_t got = 0;
    while (got < count) {
        ...
        if (pr <= 0) {
            syslog(LOG_WARNING, "[%s] recv_exact poll failed fd=%d pr=%d errno=%d (got=%zu/%zu)\n",
                TAG, fd, pr, errno, got, count);
            return -1;
        }
        int n = recv(fd, (char*)buf + got, count - got, 0);
        if (n < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
            syslog(LOG_WARNING, "[%s] recv_exact EAGAIN fd=%d (got=%zu/%zu)\n",
                TAG, fd, got, count);
            continue;
        }
        if (n <= 0) {
            syslog(LOG_WARNING, "[%s] recv_exact recv failed fd=%d n=%d errno=%d (got=%zu/%zu)\n",
                TAG, fd, n, errno, got, count);
            return -1;
        }
        got += (size_t)n;
    }
...
{
    unsigned char b0, b1;
    if (recv_exact(fd, &b0, 1) != 0) {
        syslog(LOG_WARNING, "[%s] ws_recv_frame: recv b0 failed fd=%d errno=%d\n", TAG, fd, errno);
        return -1;
    }
    if (recv_exact(fd, &b1, 1) != 0) {
        syslog(LOG_WARNING, "[%s] ws_recv_frame: recv b1 failed fd=%d errno=%d\n", TAG, fd, errno);
        return -1;
    }
    ...
    syslog(LOG_INFO, "[%s] ws_recv_frame: fd=%d b0=0x%02x b1=0x%02x opcode=%d masked=%d plen=%zu\n",
        TAG, fd, b0, b1, opcode, masked, plen);
    ...
    if (opcode == 0x9 || opcode == 0xA) {
        ...
        if (plen > 0) {
            if (recv_exact(fd, ctrl_payload, plen) != 0)
                return -1;
...
static void* client_thread(void* arg)
{
    ...
    syslog(LOG_INFO, "[%s] Client registered: %s (fd=%d), entering read loop\n", TAG,
        chat_id, fd);

    /* Send connect.challenge so Nodes can identify themselves */
#ifdef CONFIG_AI_AGENT_NODE
    syslog(LOG_INFO, "[%s] Sending challenge to fd=%d\n", TAG, fd);
    node_manager_send_challenge(fd);
    syslog(LOG_INFO, "[%s] Challenge sent to fd=%d\n", TAG, fd);
#endif
    ...
    while (s_running) {
        ...
        if (n < 0 && n != -2) {
            syslog(LOG_WARNING, "[%s] ws_recv_frame error fd=%d n=%d errno=%d\n",
                TAG, fd, n, errno);
            break; /* -2 = ignored opcode, not an error */
        }
        if (n == 0) {
            syslog(LOG_INFO, "[%s] ws_recv_frame close frame fd=%d\n", TAG, fd);
            break; /* clean close */
        }
        if (n < 0)
        ...
        if (cJSON_IsString(type_item) && strcmp(type_item->valuestring, "message") == 0 && cJSON_IsString(content_item)) {
            ...
            if (msg.content)
                message_bus_push_inbound(&msg);
                   ...
                   strcmp(type_item->valuestring, "voice_start") == 0) {
            syslog(LOG_INFO, "[%s] WS voice_start from %s\n", TAG, chat_id);
            extern int voice_channel_start(void);
            pthread_t vt;
            pthread_attr_t va;
            pthread_attr_init(&va);
            pthread_attr_setstacksize(&va, 20480);
            pthread_attr_setdetachstate(&va, PTHREAD_CREATE_DETACHED);
            int* arg = malloc(sizeof(int));
            if (arg) {
                *arg = 1;
                if (pthread_create(&vt, &va,
                        (void* (*)(void*))voice_channel_start,
                        NULL) != 0) {
                    voice_channel_start();
                }
                free(arg);
            ...
            pthread_attr_destroy(&va);
        } else if (cJSON_IsString(type_item) &&
                   strcmp(type_item->valuestring, "voice_stop") == 0) {
            /* Voice control: stop recording from external UI */
            syslog(LOG_INFO, "[%s] WS voice_stop from %s\n", TAG, chat_id);
...
int ws_server_broadcast(const char* text, const char* source_channel)
{
    if (s_listen_fd < 0 || !s_running)
        return ERROR;

    /* Build JSON response with source channel info */
    cJSON* resp = cJSON_CreateObject();
    cJSON_AddStringToObject(resp, "type", "response");
    cJSON_AddStringToObject(resp, "content", text);
    if (source_channel)
        cJSON_AddStringToObject(resp, "channel", source_channel);
    char* json_str = cJSON_PrintUnformatted(resp);
    cJSON_Delete(resp);
    if (!json_str)
        return ERROR;
    ...
    for (int i = 0; i < AGENT_WS_MAX_CLIENTS; i++) {
        if (s_clients[i].active) {
            int rc = ws_send_frame(s_clients[i].fd, json_str, strlen(json_str));
            if (rc == 0)
                sent++;
            else
                syslog(LOG_WARNING, "[%s] Broadcast send failed to %s (fd=%d)\n",
                    TAG, s_clients[i].chat_id, s_clients[i].fd);
        }
    }
    pthread_mutex_unlock(&s_clients_mtx);
    syslog(LOG_INFO, "[%s] Broadcast to %d clients: %.60s\n",
        TAG, sent, text);

    free(json_str);
    syslog(LOG_INFO, "[%s] Broadcast to %d clients\n", TAG, sent);
    return (sent > 0) ? OK : ERROR;
}

int ws_server_broadcast_typed(const char* type, const char* text,
    const char* source_channel)
{
    ...
    cJSON* resp = cJSON_CreateObject();
    cJSON_AddStringToObject(resp, "type", type ? type : "response");
    cJSON_AddStringToObject(resp, "content", text);
    if (source_channel)
        cJSON_AddStringToObject(resp, "channel", source_channel);
    char* json_str = cJSON_PrintUnformatted(resp);
    ...
    syslog(LOG_INFO, "[%s] Broadcast typed '%s' to %d clients: %.60s\n",
        TAG, type ? type : "response", sent, text);
