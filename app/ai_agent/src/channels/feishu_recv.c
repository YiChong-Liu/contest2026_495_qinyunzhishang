...
static feishu_conversation_cb_t s_conversation_cb = NULL;
static long long s_conversation_enter_time_ms = 0;  /* 进入对话模式的时间戳 */

void feishu_recv_set_conversation_callback(feishu_conversation_cb_t cb)
{
    s_conversation_cb = cb;
    if (cb) {
        /* 记录进入对话模式的时间，用于过滤历史消息 */
        s_conversation_enter_time_ms = (long long)time(NULL) * 1000LL;
        syslog(LOG_INFO, "[%s] conversation callback registered, enter_time=%lld\n",
            TAG, s_conversation_enter_time_ms);
    } else {
        s_conversation_enter_time_ms = 0;
        syslog(LOG_INFO, "[%s] conversation callback cleared\n", TAG);
    }
}

/* ── 飞书对话模式：群成员缓存 ─────────────────────────────── */

#define MEMBER_CACHE_MAX 32
typedef struct {
    char name[64];
    char open_id[64];
} member_entry_t;

static member_entry_t s_member_cache[MEMBER_CACHE_MAX];
static int s_member_count = 0;

void feishu_recv_cache_members(const char *members_json)
{
    if (!members_json || !members_json[0]) return;

    cJSON *arr = cJSON_Parse(members_json);
    ...
    cJSON_ArrayForEach(item, arr) {
        if (s_member_count >= MEMBER_CACHE_MAX) break;
        cJSON *name_j = cJSON_GetObjectItem(item, "name");
        cJSON *oid_j  = cJSON_GetObjectItem(item, "open_id");
        if (cJSON_IsString(name_j) && cJSON_IsString(oid_j)) {
            member_entry_t *e = &s_member_cache[s_member_count];
            strncpy(e->name, name_j->valuestring, sizeof(e->name) - 1);
            e->name[sizeof(e->name) - 1] = '\0';
            strncpy(e->open_id, oid_j->valuestring, sizeof(e->open_id) - 1);
            e->open_id[sizeof(e->open_id) - 1] = '\0';
            s_member_count++;
            syslog(LOG_INFO, "[%s] cached member: name=%s open_id=%s\n",
                TAG, e->name, e->open_id);
        }
    }
    cJSON_Delete(arr);
    syslog(LOG_INFO, "[%s] member cache updated: %d members\n", TAG, s_member_count);
}
...
{
    if (!name || !name[0] || s_member_count == 0) return false;

    /* 1. Exact match */
    for (int i = 0; i < s_member_count; i++) {
        if (strcmp(s_member_cache[i].name, name) == 0) {
            if (open_id_out)  strncpy(open_id_out, s_member_cache[i].open_id, oid_cap - 1);
            if (display_name_out) strncpy(display_name_out, s_member_cache[i].name, dn_cap - 1);
            syslog(LOG_INFO, "[%s] member exact match: %s -> %s\n",
                TAG, name, s_member_cache[i].open_id);
            return true;
        }
    }

    /* 2. Fuzzy match: name contains query or query contains name */
    for (int i = 0; i < s_member_count; i++) {
        if (strstr(s_member_cache[i].name, name) ||
            strstr(name, s_member_cache[i].name)) {
            if (open_id_out)  strncpy(open_id_out, s_member_cache[i].open_id, oid_cap - 1);
            if (display_name_out) strncpy(display_name_out, s_member_cache[i].name, dn_cap - 1);
            syslog(LOG_INFO, "[%s] member fuzzy match: %s -> %s (%s)\n",
                TAG, name, s_member_cache[i].name, s_member_cache[i].open_id);
            return true;
        }
    }

    syslog(LOG_WARNING, "[%s] member not found: %s\n", TAG, name);
    return false;
}
...
bool feishu_recv_find_member_by_open_id(const char *open_id,
                                         char *display_name_out, size_t dn_cap)
{
    if (!open_id || !open_id[0] || s_member_count == 0) return false;

    for (int i = 0; i < s_member_count; i++) {
        if (strcmp(s_member_cache[i].open_id, open_id) == 0) {
            if (display_name_out)
                strncpy(display_name_out, s_member_cache[i].name, dn_cap - 1);
            return true;
        }
    }
    return false;
...
static const char* extract_sender_name(cJSON *ev)
{
    static char s_sender_name[64];
    s_sender_name[0] = '\0';

    cJSON *sender = cJSON_GetObjectItem(ev, "sender");
    if (!cJSON_IsObject(sender)) return "unknown";
    ...
    if (cJSON_IsObject(sender_id)) {
        ...
        if (cJSON_IsString(uid)) {
            /* Try to match cached member by open_id */
            for (int i = 0; i < s_member_count; i++) {
                if (strcmp(s_member_cache[i].open_id, uid->valuestring) == 0) {
                    strncpy(s_sender_name, s_member_cache[i].name,
                            sizeof(s_sender_name) - 1);
                    return s_sender_name;
                }
            }
            /* Fallback: use open_id prefix */
            strncpy(s_sender_name, uid->valuestring, sizeof(s_sender_name) - 1);
            s_sender_name[sizeof(s_sender_name) - 1] = '\0';
            return s_sender_name;
        }
    }
    return "unknown";
...
static void send_data_ack(const fk_frame_t* f)
{
    ...
    if (len > 0) {
        int ret = ws_send_frame(&s_ws, 0x02, (const char*)ack_buf, (size_t)len);
        if (ret < 0) {
            /* ACK 发送失败，重试一次，减少服务器重试概率 */
            syslog(LOG_WARNING, "[%s] ACK send failed, retrying\n", TAG);
            ret = ws_send_frame(&s_ws, 0x02, (const char*)ack_buf, (size_t)len);
            if (ret < 0) {
                syslog(LOG_ERR, "[%s] ACK retry failed, server will retry event\n", TAG);
            }
        }
    }
}
...
static bool filter_stale_message(cJSON* msg)
{
    ...
    /* 设备时间明显错误（< 2020年），跳过时间过滤 */
    if (now_ms < 1577836800000LL) {  /* 2020-01-01 00:00:00 UTC */
        syslog(LOG_DEBUG, "[%s] Device time not synced (now=%lldms), skipping stale check\n",
            TAG, now_ms);
        return false;
    }

    /* 对话模式：过滤进入对话之前发送的历史消息（允许5秒容差） */
    if (s_conversation_cb && s_conversation_enter_time_ms > 0) {
        if (create_ms < s_conversation_enter_time_ms - 5000) {
            syslog(LOG_INFO, "[%s] [FEISHU_CONV] Historical msg (create=%lld < enter=%lld), dropping\n",
                TAG, create_ms, s_conversation_enter_time_ms);
            return true;
        }
        /* 绝对年龄检查：WS重连/事件重放可能投递很早之前的历史消息。
         * 飞书事件订阅投递延迟可达20s+，阈值放宽到60s避免误杀新消息。 */
        long long conv_age_ms = now_ms - create_ms;
        if (conv_age_ms > 60000) {
            syslog(LOG_WARNING, "[%s] [FEISHU_CONV] Old msg in conversation (age=%lldms >60s), dropping\n",
                TAG, conv_age_ms);
            return true;
        }
        return false;
...
static bool process_group_mentions(cJSON* msg, char** text_ptr)
{
    ...
    cJSON* mentions = cJSON_GetObjectItem(msg, "mentions");
    if (!cJSON_IsArray(mentions) || cJSON_GetArraySize(mentions) == 0) {
        /* 飞书对话模式下，允许所有群消息通过（不需要@mention） */
        if (!s_conversation_cb) {
            syslog(LOG_DEBUG, "[%s] Group msg without @mention, skipping\n", TAG);
            return true;
        }
        syslog(LOG_INFO, "[%s] [FEISHU_CONV] Group msg without @mention, forwarding (conversation mode)\n", TAG);
    }
...
void feishu_process_binary_frame(const uint8_t* data, int len, int32_t svc_id)
{
    ...
    /* ACK 必须在所有处理之前发送，减少服务器重试概率
     * 服务器未收到 ACK 会重试，重试占用配额导致新消息被丢 */
    ...
    if (cJSON_IsObject(hdr)) {
        ...
        if (cJSON_IsString(eid) && eid->valuestring[0]) {
            if (dedup_check_and_record(eid->valuestring)) {
                syslog(LOG_INFO, "[%s] Duplicate event_id=%s, skipping\n",
                    TAG, eid->valuestring);
                goto cleanup;
            }
        }
    ...
    /* Check event type */
    {
        ...
        if (strcmp(event_type, "im.message.receive_v1") != 0) {
            syslog(LOG_INFO, "[%s] ignored event_type=%s\n", TAG, event_type);
            goto cleanup;
        }
        syslog(LOG_INFO, "[%s] Received event: event_type=%s\n", TAG, event_type);
    }
    ...
    /* Text/Post message handling */
    {
        ...
        /* 内容去重：对话模式下跳过（依赖message_id去重即可，避免误判用户连续发的不同消息） */
        if (!post_image_key[0] && !s_conversation_cb && content_dedup_check(chat_id_j->valuestring, text)) {
            free(text);
            text = NULL;
            goto cleanup;
        }
        ...
        /* 飞书对话模式：消息直接通过回调送 TTS+UI，不走 agent_bus/LLM */
        if (s_conversation_cb) {
            const char *sender_name = extract_sender_name(ev);
            syslog(LOG_INFO, "[%s] [FEISHU_CONV] delivering to conversation callback: sender=%s\n",
                TAG, sender_name);
            s_conversation_cb(chat_id_j->valuestring, sender_name, text);
            free(text);
            text = NULL;
            goto cleanup;
        }
