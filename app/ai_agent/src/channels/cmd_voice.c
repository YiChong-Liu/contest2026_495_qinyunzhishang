...
void cmd_set_dashscope_key(int argc, char** argv)
{
    if (argc < 2) {
        printf("Usage: set_dashscope_key <api_key>\n");
        return;
    }
    claw_config_set(AGENT_CFG_KEY_DASHSCOPE_API_KEY, argv[1]);
    printf("DashScope API key saved (shared by Qwen3 ASR + TTS).\n");
}
