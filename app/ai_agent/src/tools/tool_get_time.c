...
static bool parse_and_set_time(const char *date_str, char *out, size_t out_size)
{
...
#else
    /* Portable fallback: mktime uses TZ env, so set UTC temporarily */
    setenv("TZ", "UTC0", 1);
    tzset();
    ...
    /* Format in local time using agent_localtime-style offset */
    struct tm local;
    time_t local_epoch = t + agent_tz_offset_sec();
    gmtime_r(&local_epoch, &local);

    char time_str[64];
    strftime(time_str, sizeof(time_str), "%Y-%m-%d %H:%M:%S (%A)", &local);
    snprintf(out, out_size, "%s (%s), UNIX epoch: %lld", time_str, agent_get_timezone(), (long long)t);
...
int tool_get_time_execute(const char *input_json, char *output, size_t output_size)
{
    ...
    if (now > 1735689600) {  /* 2025-01-01 00:00:00 UTC */
        struct tm utc_tm = agent_localtime();

        char time_str[64];
        strftime(time_str, sizeof(time_str), "%Y-%m-%d %H:%M:%S", &utc_tm);
        snprintf(output, output_size,
                 "%s (%s), UNIX epoch: %lld",
                 time_str, agent_get_timezone(), (long long)now);
