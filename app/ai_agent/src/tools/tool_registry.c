...
int tool_registry_init(void)
{
    ...
    /* Feishu today's messages: list + summarize */
    REGISTER_TOOL_NO_PARAMS("feishu_message_today",
        "List today's messages from all Feishu chats the bot joined. "
        "Returns plain text grouped by chat, ready for LLM summarization.",
        tool_feishu_message_today_execute);
