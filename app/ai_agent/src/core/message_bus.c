...
static void msg_queue_init(msg_queue_t *queue)
{
    ...
    /* Use CLOCK_MONOTONIC for condvar timeouts to avoid issues with
     * wall-clock jumps (NTP sync, timezone changes) that can cause
     * pthread_cond_timedwait to hang or misfire with CLOCK_REALTIME.
     * This is a defensive fix: the outbound dispatch thread was observed
     * to stop processing messages after WiFi disconnect/reconnect,
     * possibly due to CLOCK_REALTIME-related condvar issues. */
    pthread_condattr_t attr;
    pthread_condattr_init(&attr);
    pthread_condattr_setclock(&attr, CLOCK_MONOTONIC);
    pthread_cond_init(&queue->not_empty, &attr);
    pthread_cond_init(&queue->not_full, &attr);
...
int message_bus_push_outbound(const agent_msg_t *msg)
{
    ...
    int ret = msg_queue_push(&g_outbound_queue, msg, AGENT_BUS_PUSH_TIMEOUT_MS);
    syslog(LOG_INFO, "[%s] push_outbound: ch=%s ret=%d\n",
        TAG, msg->channel, ret);
    return ret;
}
